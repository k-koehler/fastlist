...
class ModernList<T = any> {
  ...
  public static from<T>(arr: readonly T[]): ModernList<T> {
   ...
   */
  public push(value: T): ModernList<T> {
   ...
   * @returns the modified list
   */
  public pushHead(value: T): ModernList<T> {
    this.invalidateCache();
   ...
   */
  public pushAfter(idx: number, value: T): Nullable<ModernList<T>> {
   ...
   */
  public set(idx: number, value: T): Nullable<ModernList<T>> {
    if (!this.isValidIndex(idx)) {
      return null;
    }
    const [, cur] = this.find(idx);
   ...
   */
  public map<U>(callback: (value: T) => U): ModernList<U> {
    const lst = new ModernList<U>();
    let cur = this.head;
    for (; cur !== null; cur = cur.next) {
   ...
   * @param callback the function to apply to the list
   */
  public filter(callback: (value: T) => boolean): ModernList<T> {
    const lst = new ModernList<T>();
    let cur = this.head;
    for (; cur !== null; cur = cur.next) {
